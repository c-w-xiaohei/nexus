...
- Observe an existing ordinary unicast root proxy with static `Nexus.getProxyStatus(proxy)` and future-only `Nexus.subscribeProxyStatus(proxy, listener)`. Re-read status in the listener; the subscription neither releases nor recovers the proxy.
