...
- Keep `useRemoteStore(...)` and `useStoreSelector(...)` as the lower-level React path for components that create their own `RemoteStore` handle or need the full `UseRemoteStoreResult`.
