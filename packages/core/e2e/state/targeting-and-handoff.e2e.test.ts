...
describe("Nexus State E2E: Targeting and Identity Handoff", () => {
  it("marks existing dynamic-matcher store handle stale after active-target identity handoff", async () => {
    ...
    const definition = defineNexusStore<CounterState, CounterActions>({
      token: new Token("state:counter:dynamic-active-handoff:e2e"),
      state: () => ({ count: 0 }),
  ...
  it("keeps a fixed-target store handle ready on unrelated identity updates", async () => {
    ...
    const definition = defineNexusStore<CounterState, CounterActions>({
      token: new Token("state:counter:fixed-target-stability:e2e"),
      state: () => ({ count: 0 }),
      actions: ({ getState, setState }) => ({
        increment(by: number) {
          setState({ count: getState().count + by });
