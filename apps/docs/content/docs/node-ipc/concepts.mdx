...
process, start the daemon listener first, spawn the client, and wait with
`daemonNexus.select(WorkerToken, { where, wait: { timeout: 10_000 } })`.
...
Match the spawned client's unique identity in `where`. The selection waits for
authorization and provider publication, not merely socket open. `connectTo` is
one-shot, does not block `ready()`, does not retry, and does not become the
client's `defaultTarget`; the daemon need not expose a service.
