---
title: Concepts
description: Understand daemon, client, socket, and session behavior in Node IPC.
---

This page explains the adapter mental model. Read [Quick start](/nexus/docs/node-ipc/quick-start/) first if you want working code before concepts.
...
- gets Nexus proxies from an acquired connection
...
### Client-Provided Services And Startup Dials

A client can provide services to its daemon. If the daemon creates the client
process, start the daemon listener first, spawn the client, and passively wait
with `daemonNexus.connect({ where, timeout: 10_000 })`.
...
Match the spawned client's unique identity in `where`. Passive connection waits
for an authorized ready session, not provider discovery or merely socket open.
Register services before the client publishes its application-ready identity
marker when the daemon needs those services immediately. `connectTo` is one-shot,
does not block `ready()`, does not retry, and does not acquire a service; the
daemon need not expose a service.
