...
description: Use Nexus instances, service proxy status, and State from React.
---
...
Observe an existing service proxy returned by `connection.get` or a legacy
unicast migration entry. The hook does not need `NexusProvider` and does not
...
[Service Proxy Lifecycle](/nexus/docs/proxy-lifecycle/) for the Core status and
replacement rules.
...
The planned React 0.8 package requires `@nexus-js/core` ~1.2.0 for both service
