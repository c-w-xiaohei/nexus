---
title: Identity And Connection Metadata
description: Distinguish declared identity from adapter-observed connection facts.
---
...
special core routing or authorization field. Filter with
`where: (meta) => meta.groups?.includes("workers") === true`. Identity updates
can make a session newly eligible for `onConnect`, but never alter an existing
`ConnectionCollection` snapshot.
...
`ConnectionTarget` is the adapter's exact connection input. `connect` and
`connectMulticast` acquire targets asynchronously. Neither changes the meaning
of `ContextMeta`.

`where` receives each selected connection's peer-declared context identity and local adapter facts:

```ts
const clients = await appNexus.connectMulticast({
...
The target match and `where` predicate are strict AND conditions. A targetless
`connectMulticast` binds a current ready-connection snapshot and never discovers
or connects to a new provider. Call it again to include later connections.
...
Use `updateIdentity()` only for local endpoint identity changes that affect routing, policy, diagnostics, or lifecycle behavior. It does not mutate existing `ConnectionMeta`, change an existing session's target, or rebind old proxies. Observe a peer's current and future complete identity with `connection.subscribeIdentity(listener)`; it does not deduplicate fields or turn updates into a second `onConnect` event.
