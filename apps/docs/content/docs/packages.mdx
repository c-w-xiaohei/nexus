---
title: Nexus Package Map
description: Choose Nexus packages and imports for each runtime and integration.
---
...
- Add `@nexus-js/react` when React components need a Nexus instance or Nexus State
...
<InstallCommand packages="@nexus-js/core @nexus-js/react" />
...
- `@nexus-js/react`
  - React bindings for Nexus instances and Nexus State
  - Provides `createRemoteStoreScope(...)` for shared `RemoteStore` handles and lower-level State hooks
  - See [React](/nexus/docs/react/) for general React integration and [Nexus State React](/nexus/docs/state/react/) for Nexus State
...
`@nexus-js/core/state`, `@nexus-js/core/relay`, `@nexus-js/core/transport`, and
`@nexus-js/core/transport/virtual-port` are imported from the installed core
package; none is a separately installed package.
