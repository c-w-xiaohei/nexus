...
description: Reference the headless Nexus State store and subscription APIs.
---
...
- connection targeting is supplied by the State connection options, not by the StoreToken
...
adapter association and optional `validation: { state, actionResults }` schemas.
Use `space.storeToken<Store>(name, options?)` or `space.safeStoreToken<Store>(...)`
with TokenSpace to inherit namespaced IDs. A local store may contain additional
private fields; `snapshot` and `expose` still define the runtime sharing boundary.

### Validation schemas

State and action-result validators accept synchronous
[Standard Schema](https://standardschema.dev/) implementations, including
Valibot, Zod, and Zod Mini. Nexus uses Valibot for its own protocol schemas;
applications can keep their preferred validator.
...
Validators must check the actual snapshot or action-result shape. Nexus preserves
the original value: defaults and transformed outputs are not installed in a
mirror or returned instead of an action result. A coercing or transforming
validator's output type does not establish the original value's type.
The TypeScript contract constrains the validator's output type; it does not
statically check input compatibility. Unknown-input validators are supported.

Validation must return synchronously. Asynchronous validators are rejected with
`E_STORE_PROTOCOL`; they do not delay publication. Thrown validation errors are
also converted to State protocol errors.
...
- accepts Core `ConnectOptions`: target, where, positive finite timeout, and signal
- acquires a connection, then checks its current service catalog; it does not wait for a missing provider
...
Each subscription uses an explicit Core resource scope automatically. Destroying
the mirror closes that scope without closing the shared Connection. A Relay path
uses the same scope end to end; no `relayNexusStore` helper is needed.
...
Timeout boundaries remain separate: a supplied `timeout` bounds connection
acquisition and then separately bounds callback-init setup. It is not one shared
end-to-end deadline. `signal` cancels connection acquisition only; it does not
cancel a subscription handshake that has already started or destroy a returned store.
Action calls use Core's existing resource-call timeout defaults; State does not
translate the acquisition timeout into Core's `callTimeout` option.
...
- if the supplied `where` stops matching on an identity update, the State handle becomes `stale`; ordinary Connection proxies do not retain that predicate

If its resource scope closes, the mirror is terminal. Create a new subscription
for a recovered route and fresh baseline; State does not replay actions or
snapshot acknowledgements into an old mirror. See
